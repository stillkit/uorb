...
VDev::init()
{
	...
	if (_devname != nullptr) {
		PX4_DEBUG("VDev::init %s",_devname);
...
int
VDev::poll(file_t *filep, pollfd_struct_t *fds, bool setup)
{
...
void
VDev::poll_notify_one(pollfd_struct_t *fds, pollevent_t events)
{
...
int
VDev::store_poll_waiter(pollfd_struct_t *fds)
{
	/*
	...
	pollfd_struct_t **new_pollset = new pollfd_struct_t *[new_count];
	...
	if (_max_pollwaiters > 0) {
		memset(new_pollset + _max_pollwaiters, 0, sizeof(pollfd_struct_t *) * (new_count - _max_pollwaiters));
		memcpy(new_pollset, _pollset, sizeof(pollfd_struct_t *) * _max_pollwaiters);
		delete[](_pollset);
	}
...
int
VDev::remove_poll_waiter(pollfd_struct_t *fds)
{
...
VDev *VDev::getDev(const char *path)
{
	...
	for (; i < PX4_MAX_DEV; ++i) {
		...
		if (devmap[i] && (strcmp(devmap[i]->name, path) == 0)) {
			pthread_mutex_unlock(&devmutex);
			PX4_DEBUG("VDev::getDev %s",path);
			return (VDev *)(devmap[i]->cdev);
		}
	}

	pthread_mutex_unlock(&devmutex);
PX4_DEBUG("VDev::getDev nullptr");
	return nullptr;
}
